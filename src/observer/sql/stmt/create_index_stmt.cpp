...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) || create_index.attributes_names.size() == 0) {
  ...
  // 根据create_index中的属性，从table_meta中找出对应的field_meta
  std::vector<std::string> attrs_names;
  for(int i = 0 ; i < create_index.attributes_names.size() ; i ++ ) {
    attrs_names.emplace_back(create_index.attributes_names[i].attribute_name);
  }
  std::vector<FieldMeta> index_fields;
  table->table_meta().fields_by_attrs(index_fields, attrs_names);
  if(index_fields.size() == 0) {
  ...
  stmt = new CreateIndexStmt(table, index_fields, create_index.index_name);
  return RC::SUCCESS;
}
